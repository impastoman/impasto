...
struct ProcessScriptStepView: View {
    ...
    var pickSection: some View {
        Section {
            ...
            if store.savedProcesses.isEmpty {
                Text("No saved processes yet â€” create one below or from the Library.")
    ...
    var saveToLibrarySection: some View {
        Section {
          ...
          footer: { Text("Optional â€” save this process for reuse in future recipes").tipText() }
        .listRowBackground(Color.clear)
    }

    func positionLabel(for idx: Int) -> String {
        if processCards[idx].type == .combine { return "ðŸ”’" }
        return "\(idx)"
    }
}
